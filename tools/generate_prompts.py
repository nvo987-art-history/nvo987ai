...
def validate_dataset(prompts):
    ...
    if len(prompts) != TOTAL_PROMPTS:
        raise ValueError("Dataset size validation failed.")
...
def build_card(item):
    ...
    description = safe_html(item["description"])
    category = safe_html(item["category"])
    tags = safe_html(", ".join(item["tags"]))
...
<section class="grid" id="promptGrid">
...
def main():
    ...
    print(f"Prompt pages: {len(generated_urls)}")
